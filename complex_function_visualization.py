...
class ComplexFunctionVisualization(Scene):
    ...
    def demonstrate_path(self, complex_plane, function, path_type_text, path_name, path_creator):
        ...
        self.update_subtitle(f"现在是{path_name}", wait=2)
    ...
    def construct(self):
        ...
        # 添加字幕介绍
        self.update_subtitle("欢迎来到复函数可视化演示", wait=3.0)
        self.update_subtitle(f"我们将在复平面上探索复变函数及其导数的行为", wait=5.0)
        
        # 添加复平面和函数公式
        self.update_subtitle("首先让我们创建复平面", wait=1)
        self.play(Create(complex_plane), run_time=2.0)
        self.animation_timer += 2.0
        
        self.update_subtitle("这是我们要研究的复函数", wait=1)
        self.play(Write(function_formula), run_time=1.5)
        self.animation_timer += 1.5
        
        self.update_subtitle("第一个问题是：当自变量沿复平面上的特定路径移动时，函数值会怎样移动？", wait=7.5)
        self.update_subtitle("接下来自变量和函数值将分别沿着红色、黄色路径移动，请注意观察。", wait=7.5)
        
        self.demo_path_combined(complex_plane, path_type, complex_function1)
        
        # 5. 展示导数可视化
        self.update_subtitle("现在我们继续探索复函数的导数特性", wait=4.0)

        self.update_subtitle(f"接下来大家将依次看到两个彩色环，它们分别表示自变量和函数值的微小变化", wait=8)

        self.update_subtitle("当然，为了能看清，两个环会被放大同样的倍数。", wait=5)

        self.update_subtitle("环在不同角度具有不同颜色，相同颜色的 df 和 dz 是一对", wait=7)

        self.update_subtitle("如果复变函数可导，df 和 dz 的比值就是常数", wait=6)

        self.update_subtitle("那么 df 环应该也是圆的，而且颜色和 dz 圆环只差一个固定的角度", wait=1)
        ...
        self.update_subtitle("然后是导数可视化，这个时候请注意 df 就不一定是圆环了", wait=2)
        self.demonstrate_derivative(complex_plane, complex_function3)
    
        self.update_subtitle("演示结束，感谢观看！", wait=3.0)
...
if __name__ == "__main__":
    print("请使用以下命令运行动画:")
    print("manim -ql complex_function_visualization.py ComplexFunctionVisualization") 